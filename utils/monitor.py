...
class Monitor():
    ...
    def show_val_result_downstream(self):
        for image_box in self.val_image_list:
            plt.figure(figsize=(12,4))
            plt.subplot(2, 5, 1)
            plt.imshow(image_box[0].cpu().detach().numpy()[0][0], 'gray', vmin=0.0, vmax=1.0)
            plt.title('Moving Org Image')
            plt.axis('off')

            plt.subplot(2, 5, 2)
            plt.imshow(image_box[1].cpu().detach().numpy()[0][0], 'gray', vmin=0.0, vmax=1.0)
            plt.title('Moving Rec Image')
            plt.axis('off')

            plt.subplot(2, 5, 3)
            plt.imshow(image_box[2].cpu().detach().numpy()[0][0], 'gray', vmin=0.0, vmax=1.0)
            plt.title('Moving Lower Layer')
            plt.axis('off')

            plt.subplot(2, 5, 4)
            plt.imshow(image_box[2].cpu().detach().numpy()[0][1], 'gray', vmin=0.0, vmax=1.0)
            plt.title('Moving Upper Layer')
            plt.axis('off')

            plt.subplot(2, 5, 5)
            plt.imshow(image_box[6].cpu().detach().numpy()[0][0], 'gray', vmin=0.0, vmax=1.0)
            plt.imshow(image_box[6].cpu().detach().numpy()[0][1], 'gray', alpha=0.5, vmin=0.0, vmax=1.0)
            plt.title('Moving Warpped Image')
            plt.axis('off')

            plt.subplot(2, 5, 6)
            plt.imshow(image_box[3].cpu().detach().numpy()[0][0], 'gray', vmin=0.0, vmax=1.0)
            plt.title('Fixed Org Image')
            plt.axis('off')

            plt.subplot(2, 5, 7)
            plt.imshow(image_box[4].cpu().detach().numpy()[0][0], 'gray', vmin=0.0, vmax=1.0)
            plt.title('Fixed Rec Image')
            plt.axis('off')

            plt.subplot(2, 5, 8)
            plt.imshow(image_box[5].cpu().detach().numpy()[0][0], 'gray', vmin=0.0, vmax=1.0)
            plt.title('Fixed Lower Layer')
            plt.axis('off')

            plt.subplot(2, 5, 9)
            plt.imshow(image_box[5].cpu().detach().numpy()[0][1], 'gray', vmin=0.0, vmax=1.0)
            plt.title('Fixed Upper Layer')
            plt.axis('off')

            plt.subplot(2, 5, 10)
            plt.imshow(image_box[7].cpu().detach().numpy()[0][0])
            plt.imshow(image_box[7].cpu().detach().numpy()[0][1], alpha=0.5)
            plt.title('Loss Image')
            plt.axis('off')

            plt.show()
            print(image_box[8].cpu().detach().numpy()[0][0])
    ...
    def show_val_result(self):
        for image_box in self.val_image_list:
            ...
            plt.imshow(image_box[0].cpu().detach().numpy()[0][0], 'gray', vmin=0.0, vmax=1.0)
            if len(image_box) > 5:
                flag = image_box[5].cpu().detach().numpy()[0]
                plt.title('Original Image {}'.format(flag))
            else:
                plt.title('Original Image')
            plt.axis('off')

            plt.subplot(2, 3, 4)
            plt.imshow(image_box[2].cpu().detach().numpy()[0][0] * image_box[4].cpu().detach().numpy()[0][0], 'gray', vmin=0.0, vmax=1.0)
            ...
            plt.subplot(2, 3, 3)
            plt.imshow(image_box[1].cpu().detach().numpy()[0][1], 'gray', vmin=0.0, vmax=1.0)
