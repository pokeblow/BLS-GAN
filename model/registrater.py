...
def registration(output, x1, x2):
    ...
    range_crop = 5
    x1_reg = x1_reg[:, :, range_crop: image_size - range_crop, range_crop: image_size - range_crop]
    x2 = x2[:, :, range_crop: image_size - range_crop, range_crop: image_size - range_crop]

    length = (image_size - range_crop) / 2
...
class Registrater(torch.nn.Module):
    ...
    def forward(self, x1_org, x2_org, x1_masks, x2_masks):
        '''
        ...
        output_upper = output[:, 1]
        output_lower = output[:, 0]

        # 0 -- lower, 1 -- upper
        x1_org_upper = x1_org[:, 1].unsqueeze(1)
        x2_org_upper = x2_org[:, 1].unsqueeze(1)
        x1_org_upper_mask = x1_masks[:, 1].unsqueeze(1)
        x2_org_upper_mask = x2_masks[:, 1].unsqueeze(1)
        # upper
        ...
        # lower
        x1_org_lower = x1_org[:, 0].unsqueeze(1)
        x2_org_lower = x2_org[:, 0].unsqueeze(1)
        x1_org_lower_mask = x1_masks[:, 0].unsqueeze(1)
        x2_org_lower_mask = x2_masks[:, 0].unsqueeze(1)
        ...
        x1_reg = torch.cat((x1_lower, x1_upper), dim=1)
        x2_reg = torch.cat((x2_lower, x2_upper), dim=1)

        x1_masks_reg = torch.cat((x1_lower_mask, x1_upper_mask), dim=1)
        x2_masks_reg = torch.cat((x2_lower_mask, x2_upper_mask), dim=1)
        ...
        parmeter = torch.cat((parmeter_lower, parmeter_upper), 1)

        return x1_reg, x2_reg, x1_masks_reg, x2_masks_reg, parmeter
